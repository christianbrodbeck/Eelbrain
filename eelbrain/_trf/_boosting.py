...
class Boosting:
    ...
    def fit(
        ...
        hs, hs_failed = boosting_runs_mp(self.data, split_train, split_validate, split_train_and_validate, i_start_by_x, i_stop_by_x, delta, mindelta_, error, selective_stopping)
...
def boosting_runs_mp(
    ...
    stop_jobs = mpc.Event()
    args = (y_buffer, x_buffer, x_pads_buffer, n_y, n_times, n_x, split_train, split_validate, split_train_and_validate, i_start_by_x, i_stop_by_x, delta, mindelta, error, selective_stopping, job_queue, result_queue)
...
def boosting_worker(y_buffer, x_buffer, x_pads_buffer, n_y, n_times, n_x, split_train, split_validate, split_train_and_validate, i_start_by_x, i_stop_by_x, delta, mindelta, error, selective_stopping, job_queue, result_queue):
    if CONFIG['nice']:
        os.nice(CONFIG['nice'])
...
def boosting_run(
        ...
        split_validate: np.ndarray,
        split_train_and_validate: np.ndarray,
        i_start_by_x: np.ndarray,
        i_stop_by_x: np.ndarray,
        delta: float,
        mindelta: float,
    ...
    for i_boost in range(999999):
        # evaluate current h
        e_train = error_for_indexes(y_error, split_train, error_func)
        e_test = error_for_indexes(y_error, split_validate, error_func)
...
def update_error(
        ...
        x: np.ndarray,
        x_pad: float,  # pad x outside valid convolution area
        indexes: np.ndarray,  # segment indexes
        delta: float,
        shift: int,
):
    for seg_i in range(indexes.shape[0]):
